...
# kata setelahnya, KHUSUS pada pola ini saja (bukan setiap titik dua di
# judul). Grup 1 menangkap "Bagian N"/"Part N" TANPA titik dua, supaya spasi
# sebelum ":" ikut dirapikan.
...
def _normalize_title_text(text: str) -> str:
    """
    Normalisasi format judul ISO/SNI menjadi format baku, contoh:
    ...
    Langkah:
    1. Merapikan strip panjang (en dash/em dash) jadi " — " (spasi-em
       dash-spasi), apa pun spasi aslinya (nempel salah satu/kedua sisi,
       atau spasi ganda). Hyphen pendek "-" TIDAK disentuh sama sekali.
    2. Mengkapitalisasi huruf pertama kata setelah " — " hasil rapikan.
    3. Merapikan spasi di sekitar "Bagian N:" / "Part N:" jadi tepat satu
       ...
       kata setelahnya (mis. "Bagian 1 :umum" -> "Bagian 1: Umum").
    """
    if not text:
        return text
    out = _RE_LONG_DASH.sub(' \u2014 ', text)
    out = _RE_AFTER_LONG_DASH.sub(lambda m: m.group(1) + m.group(2).upper(), out)
    out = _RE_BAGIAN_PART.sub(lambda m: m.group(1) + ': ' + m.group(2).upper(), out)
    return out


def _fix_main_title_paragraph(p):
    """
    Judul utama (halaman 1 / content) pada dokumen ISO sumber sering memuat:
    1. Line-break manual (<w:br/>) di tengah judul panjang, mis. "...— Bagian
       1:" lalu baris baru "umum". python-docx membaca <w:br/> sebagai
       karakter '\\n' literal di tiap run, sehingga jika dibiarkan judul
       akan tampak terpecah jadi beberapa baris padahal seharusnya satu
       baris penuh (sebagaimana sudah ditampilkan pada halaman cover).
    ...
    Fungsi ini, dijalankan pada paragraf judul utama itu sendiri:
    1. Menghapus seluruh <w:br/> di dalam paragraf (gabung jadi satu baris).
    2. Merapikan strip panjang & mengkapitalisasi huruf pertama tiap klausa
       terkait, lewat _normalize_title_text() di atas.

    Karena langkah-langkah ini bisa mengubah PANJANG teks (bukan cuma isi
    karakternya — mis. merapikan spasi ganda atau menyisipkan spasi yang
    hilang), hasil akhirnya ditulis ke run pertama paragraf dan run lainnya
    dikosongkan; formatting (bold/size/center) tetap diterapkan oleh caller
    setelah fungsi ini, jadi visual judul tidak berubah, hanya teksnya.
    """
    W = WNS

    # 1. Hapus semua <w:br/> di dalam paragraf (gabungkan jadi satu baris).
    #    <w:br/> bisa berdiri sendiri dalam run-nya sendiri (tanpa teks) atau
    #    bercampur dengan run yang juga punya <w:t>. Cukup hapus elemen
    #    <w:br/>-nya saja dan beri spasi tunggal sebagai pengganti baris baru,
    #    supaya kata sebelum & sesudahnya tidak menyatu tanpa spasi.
    for run in list(p.runs):
        r_el = run._element
        brs = r_el.findall(f'{{{W}}}br')
        if not brs:
            continue
        has_text = bool(r_el.findall(f'{{{W}}}t'))
        for br in brs:
            r_el.remove(br)
        # Run yang HANYA berisi <w:br/> (tanpa teks) jadi run kosong setelah
        # br dihapus — sisipkan spasi supaya kata di kedua sisi tidak nempel.
        if not has_text:
    ...
    # 2. Normalisasi teks gabungan: rapikan spasi ganda, strip panjang, dan
    #    kapitalisasi klausa — lalu tulis hasilnya ke run pertama.
    full_text = p.text
    normalized = re.sub(r'\s{2,}', ' ', full_text).strip()
    normalized = _normalize_title_text(normalized)
    if normalized != full_text and p.runs:
        p.runs[0].text = normalized
        for r in p.runs[1:]:
            r.text = ''
        ...
        try:
            ...
            for p in paragraphs:
                ...
                if not title_processed and not is_real_heading:
                    ...
                    # Gabungkan line-break manual jadi satu baris & kapitalisasi
                    # "Bagian N: x" -> "Bagian N: X", sebelum formatting diterapkan,
                    # agar judul halaman 1 konsisten dengan judul di cover.
                    _fix_main_title_paragraph(p)
                    clean_format(p)
                    for run in p.runs:
                        run.bold = True
                        run.font.size = Pt(12)
