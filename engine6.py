...
def _para_notranslate(runs_xml: str, align='center', space_after=0, indent_left=0) -> str:
    """
    Sama seperti _para(), tapi menyisipkan <w:pStyle w:val="RSNINoTranslate"/>.
    Dipakai KHUSUS untuk heading baku "Prakata" dan "Pendahuluan" yang
    disisipkan engine6 (sudah berbahasa Indonesia final). Engine9
    (DocxFinalTranslatorEngine) mengenali style id ini dan melewatkan
    paragraf dari proses translate, supaya kata pendek seperti "Prakata"
    tidak salah dideteksi bahasanya oleh Google Translate lalu berubah
    (misal jadi "Foreword"). Hanya dipakai untuk paragraf heading ini —
    paragraf isi Prakata/Pendahuluan lainnya tetap memakai _para() biasa.
    """
    jc  = f'<w:jc w:val="{align}"/>'
    sa  = f'<w:spacing w:before="0" w:after="{space_after}"/>'
    ind = f'<w:ind w:left="{indent_left}"/>' if indent_left else ''
    return (
        f'<w:p><w:pPr>{sa}{jc}{ind}'
        f'<w:pStyle w:val="RSNINoTranslate"/>'
        f'<w:rPr><w:rFonts w:ascii="Arial" w:eastAsia="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:noproof/></w:rPr>'
        f'</w:pPr>{runs_xml}</w:p>'
    )
...
def _build_prakata(sni_number, title_id, title_en, ref_standard, bsn_year, num_id: int = 1):
    ...
    xmls.append(_para_notranslate(_run('Prakata', bold=True, size_pt=12), align='center'))
...
def _build_pendahuluan():
    xmls = []
    xmls.append(_page_break_para())
    xmls.append(_para_notranslate(_run('Pendahuluan', bold=True, size_pt=12), align='center'))
    xmls.append(_empty(align='center'))
    xmls.append(_empty(align='center'))
    xmls.append(_empty(align='center'))
