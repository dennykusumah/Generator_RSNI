"""
Engine9: DocxFinalTranslatorEngine + 2 Spreadsheet Terpisah
============================================================================
FIX v2:
  - FIX CRITICAL: Hyperlink handling sekarang AMAN, tidak merusak struktur XML
  - 2 SPREADSHEET TERPISAH:
    1. KAMUS_SPREADSHEET_URL → untuk terjemahan istilah (CustomDictionary/Kamus SNI)
...
FIX v3 — Perbaikan menyeluruh:
  BUG 2 FIXED (KRITIS): ItalicDictionary (kamus istilah asing) sekarang benar:
    Root cause: Token @@IT_XXXX@@ dikirim ke Google Translate yang
    mengubah formatnya (spasi, huruf kecil, dll.) sehingga _apply_post
    tidak bisa menemukannya kembali → term tidak di-restore → tidak miring.
    Fix: Ganti token @@IT_...@@ dengan placeholder §N§ (\xa7N\xa7) yang
    TIDAK diubah oleh Google Translate. Restore dilakukan setelah API,
    SEBELUM early-return. Term yang ditemukan dimiringkan di output.

  BUG 3 FIXED: CustomDictionary (kamus SNI) tidak mengganggu italic dict:
    Dua lapisan tokenisasi dijalankan secara berurutan dan independen.
    Jika seluruh teks ada di kamus, API tidak dipanggil (efisien).

FIX v4 — Perbaikan heading & token kamus SNI:
  BUG 4 FIXED (KRITIS, REGRESI dari v3): Heading "Foreword", "Introduction",
    "Scope", dan Heading 1-6 lainnya SEBELUMNYA di-skip TOTAL dari proses
    translate (perbaikan v3 yang salah arah). Akibatnya heading tertinggal
    dalam bahasa Inggris, ATAU — jika lolos dari deteksi skip — diterjemahkan
    mentah oleh Google Translate tanpa kamus SNI (mis. "Foreword" jadi
    "Kata Pengantar", bukan "Prakata" sesuai Kamus SNI di Google Sheet).
    Fix: heading TIDAK lagi di-skip; ia melewati alur _translate_para yang
    SAMA dengan paragraf biasa, sehingga custom_dict (Kamus SNI) selalu
    sempat menggantikan istilah sebelum teks dikirim ke Google Translate.

  BUG 5 FIXED: CustomDictionary (Kamus SNI) memakai token @@TK_xxxxxxxx@@
    yang mengandung simbol "@@" ganda — berisiko sama dengan BUG 2 (rawan
    diubah API translate). Diganti dengan placeholder §TKn§ (format yang
    sama terbukti aman dipakai ItalicDictionary), plus fallback regex
    toleran-spasi di _apply_post sebagai jaring pengaman tambahan.

  BUG 6 FIXED: app.py tidak mem-pass italic_dict ke DocxFinalTranslatorEngine
    saat proses translate dijalankan (hanya custom_dict yang di-pass),
    sehingga proteksi italic tidak pernah aktif. italic_dict kini diteruskan.

  PERUBAHAN: Auto-refresh kamus dari Google Sheet — TIDAK ADA lagi tombol
    refresh manual. Kamus SNI & Kamus Istilah Asing di-fetch ulang dari
    Google Sheet setiap kali halaman dimuat dan sesaat sebelum proses
    translate dijalankan, sehingga perubahan terbaru di spreadsheet selalu
    terpakai tanpa aksi tambahan dari pengguna.
"""
...
class CustomDictionary:
    ...
    def _apply_pre(self, text: str) -> tuple[str, dict]:
        """
        Ganti istilah kamus dengan placeholder numerik \xa7N\xa7 (§N§).
        CATATAN: placeholder ini SENGAJA dibuat mirip dengan skema italic
        dict (bukan token @@TK_xxxxxxxx@@ seperti versi lama) karena simbol
        '@@' ganda terbukti rawan diubah/dipecah oleh Google Translate
        (spasi disisipkan, huruf besar/kecil berubah, dll.) sehingga restore
        di _apply_post gagal dan istilah kamus SNI hilang/tidak terganti.
        Placeholder §N§ (karakter section sign + digit) jauh lebih jarang
        diutak-atik oleh mesin terjemahan karena tidak menyerupai markup.
        """
    ...
    def _apply_post(self, translated: str, token_map: dict) -> str:
        ...
        for token, tgt in token_map.items():
            # Exact match dulu (kasus normal: placeholder lolos tidak berubah)
            if token in result:
                result = result.replace(token, tgt)
                continue
            # Fallback toleran: beberapa mesin translate menyisipkan spasi
            # di sekitar simbol non-alfanumerik (mis. "§ TK0 §" / "§TK0 §").
            # Cari pola \xa7\s*TK<idx>\s*\xa7 sebagai jaring pengaman.
            m = re.search(r'TK(\d+)', token)
            if m:
                loose_pattern = re.compile(r'\xa7\s*TK' + m.group(1) + r'\s*\xa7', re.IGNORECASE)
                result = loose_pattern.sub(tgt, result)
        return result
...
class ItalicDictionary:
    ...
    def _apply_pre(self, text: str) -> tuple[str, dict]:
        """
        CATATAN: alur aktif (_translate_para / _translate_hyperlinks_in_para)
        TIDAK memanggil method ini lagi — keduanya membangun placeholder §N§
        secara inline langsung dari self._entries. Method ini dipertahankan
        untuk kompatibilitas pemanggil lain, dan diperbaiki agar konsisten:
        placeholder numerik §N§, BUKAN token @@IT_xxxxxxxx@@ (terbukti rawan
        diubah/dipecah oleh Google Translate sehingga restore gagal).
        """
        if not self._entries: return text, {}
        token_map = {}
        result = text
    ...
    def _apply_post(self, translated: str, italic_map: dict) -> tuple[str, list[str]]:
        ...
        for token, original in italic_map.items():
            if token in result:
                ...
                # Fallback toleran terhadap spasi ekstra di sekitar §N§
                m = re.search(r'(\d+)', token)
                if m:
                    loose_pattern = re.compile(r'\xa7\s*' + m.group(1) + r'\s*\xa7')
                    if loose_pattern.search(result):
                        result = loose_pattern.sub(original, result)
                        if original not in italic_terms_found:
                            italic_terms_found.append(original)
...
def _skip_paragraph(para, past_bibliography: bool = False) -> bool:
    ...
    # Heading baku "Prakata"/"Pendahuluan" yang disisipkan engine6 (template
    # BSN, sudah final dalam Bahasa Indonesia) ditandai dengan pStyle khusus
    # "RSNINoTranslate" supaya kata pendek seperti "Prakata" tidak salah
    # dideteksi bahasanya oleh Google Translate lalu berubah (misal jadi
    # "Foreword"). HANYA dua heading ini yang di-skip — paragraf isi
    # Prakata/Pendahuluan lainnya tetap memakai style "Normal" dan tetap
    # melewati alur translate seperti biasa.
    if _get_para_style_id(para) == 'RSNINoTranslate':
        return True
    # CATATAN: Heading 1-6, "Foreword", "Introduction", dll. (milik dokumen
    # ISO sumber) TIDAK di-skip di sini. Heading-heading itu justru WAJIB
    # melewati alur translate normal (_translate_para) agar Kamus SNI
    # (custom_dict) bisa menangkapnya, misal "Foreword" -> "Prakata",
    # "Introduction" -> "Pendahuluan", "Scope" -> "Ruang Lingkup". Men-skip
    # heading akan membuatnya tertinggal dalam bahasa Inggris, dan TIDAK
    # men-skip tanpa kamus akan membuatnya diterjemahkan mentah oleh Google
    # Translate (misal jadi "Kata Pengantar" bukan "Prakata"). Style Annex
    # juga ditangani di blok tersendiri.
    return False
...
def _translate_hyperlinks_in_para(para, tr) -> None:
    ...
    FIX: Italic dict dilindungi dengan placeholder §N§ (bukan @@IT_...@@)
         sehingga Google Translate tidak mengubah/merusaknya.
    """
    p_el = para._element
    ...
    for hl in p_el.findall(f'{_W}hyperlink'):
        ...
        # Proteksi italic dict dengan §N§
        if tr.italic_dict and len(tr.italic_dict) > 0:
            sorted_italic = sorted(
                tr.italic_dict._entries.items(),
                key=lambda x: len(x[0]), reverse=True
            )
            for term_lower, term_orig in sorted_italic:
                pattern = re.compile(
                    r'(?<![A-Za-z0-9\u00C0-\u024F])' + re.escape(term_lower) +
                    r'(?![A-Za-z0-9\u00C0-\u024F])',
                    re.IGNORECASE
                )
                def _replacer(m, _orig=term_orig):
                    idx = len(italic_slots)
                    italic_slots.append(_orig)
                    return f'\xa7{idx}\xa7'
                working = pattern.sub(_replacer, working)

        # Proteksi custom dict
        tk_map = {}
        if tr.custom_dict and len(tr.custom_dict) > 0:
            working, tk_map = tr.custom_dict._apply_pre(working)

        # Terjemahkan jika ada teks nyata
        probe = re.sub(r'\xa7TK\d+\xa7', '', working)
        probe = re.sub(r'\xa7\d+\xa7', '', probe).strip()
        _ONLY_PUNCT = re.compile(
            r'^[\s\d\.\,\:\;\-\(\)\[\]\/\\\+\=\*\%\&\^\$\#\@\!\"\'`~<>{}|_©]*$'
        )

        translated = working
        if probe and not _ONLY_PUNCT.fullmatch(probe):
            translated, _ = tr.translate_one(working, {})

        # Restore custom dict
        if tk_map:
            translated = tr.custom_dict._apply_post(translated, tk_map)

        # Restore italic §N§ → teks asli
        def _restore_italic(m):
            try:
                return italic_slots[int(m.group(1))]
            except (IndexError, ValueError):
                return m.group(0)
        translated = re.sub(r'\xa7(\d+)\xa7', _restore_italic, translated)

        # Tulis kembali ke elemen <w:t> (AMAN)
        if t_els:
...
# ─────────────────────────────────────────────────────────────────────────────
# FITUR 4: SUPERSCRIPT PANGKAT / KUADRAT / KUBIK
# ─────────────────────────────────────────────────────────────────────────────
# Empat sumber angka pangkat / notasi terkait yang perlu diproses:
#   (a) Satuan baku yang diikuti pangkat 2/3, mis. "m2", "cm3", "mm2", "km2"
#       -> "m²", "cm³", "mm²", "km²" (digit pangkat dijadikan superscript).
#   (b) Notasi eksplisit dengan tanda "^", mis. "10^2", "x^3"
#       -> simbol "^" dihapus, angka/variabel setelahnya dijadikan superscript.
#   (c) Superscript asli dari dokumen sumber (mis. digit pangkat yang sudah
#       diberi w:vertAlign="superscript" oleh penulis ISO) yang HILANG karena
#       proses translate membongkar-ulang seluruh run dalam paragraf menjadi
#       satu run teks polos. Karena info itu sudah tidak bisa dilacak setelah
#       translate, kasus (c) ditangani dengan mendeteksi ULANG pola pangkat
#       yang sama seperti (a)/(b) pada teks hasil terjemahan — sehingga hasil
#       akhirnya tetap benar terlepas dari apakah sumbernya berupa run
#       superscript asli atau teks digit biasa.
#   (d) Notasi derajat "oC" / "oF" (huruf "o" superscript di depan C/F, gaya
#       penulisan lama sebelum simbol "°" lazim dipakai) -> disubstitusi
#       menjadi simbol derajat baku "°C" / "°F" (bukan superscript, melainkan
#       penggantian karakter).

# Satuan metrik yang lazim diikuti pangkat luas (²) atau volume (³).
_SUPERSCRIPT_UNITS = (
    r'(?:[nµumcdk]?m|in|ft|yd|mi)'  # nm, µm, um, mm, cm, dm, km, m, in, ft, yd, mi
)
# (a) satuan + digit pangkat (2 atau 3), tidak diikuti huruf/digit lain
#     supaya tidak salah kena "m20" atau "cm2x".
_RE_UNIT_POWER = re.compile(
    r'(?<![A-Za-z0-9])(' + _SUPERSCRIPT_UNITS + r')([23])(?![A-Za-z0-9])'
)
# (b) notasi eksplisit "^": dasar (angka/variabel) + "^" + eksponen (angka,
#     boleh negatif, mis. 10^-3).
_RE_CARET_POWER = re.compile(r'(\d+(?:[.,]\d+)?|[A-Za-z])\^(-?\d+)')
# (e) notasi pangkat negatif inline seperti "kWm−2", "W·m−2", "kg·m−3",
#     "s−1", dll. — unit/huruf diikuti tanda minus (biasa "-" atau unicode
#     "−" U+2212) dan satu atau lebih digit, TANPA spasi di antaranya.
#     Contoh: "20 kWm−2" → "20 kWm" + superscript "−2"
#              "W·m-2"   → "W·m"    + superscript "-2"
#     Tanda minus pada pangkat DIPERTAHANKAN di superscript (tidak dihapus).
_RE_INLINE_NEG_POWER = re.compile(
    r'(?<=[A-Za-zµΩ·⁻])([\u2212\-]\d+)(?![A-Za-z0-9\.])'
)
# (d) notasi derajat lama "oC" / "oF": huruf "o" tepat sebelum C/F, didahului
#     spasi/digit (mis. "25 oC", "100oC"), tidak diikuti huruf lain (supaya
#     tidak salah kena kata biasa seperti "oCean" -> tidak ada huruf
#     setelahnya selain akhir kata/boundary).
_RE_DEGREE_OLD = re.compile(r'(?<=[\d\s])o([CF])(?![A-Za-z])')


def _normalize_degree_notation(text: str) -> str:
    """Ganti notasi derajat lama 'oC'/'oF' menjadi simbol baku '°C'/'°F'."""
    return _RE_DEGREE_OLD.sub(lambda m: '\u00b0' + m.group(1), text)


def _apply_superscript_to_para(para) -> bool:
    """
    Pindai teks gabungan sebuah paragraf untuk pola pangkat (a) dan (b) di
    atas, lalu tulis ulang paragraf tersebut sebagai rangkaian run dengan
    bagian pangkatnya diberi format superscript (w:vertAlign).
    ...
    # (d) Substitusi notasi derajat lama "oC"/"oF" -> "°C"/"°F" lebih dulu.
    # Penggantian 1 karakter ('o') dengan 1 karakter ('°') per kemunculan,
    # sehingga tidak mengubah panjang/posisi teks dan aman dilakukan sebelum
    # pemindaian span pangkat (a)/(b) di bawah.
    full_text = _normalize_degree_notation(full_text)

    # Deteksi cepat: ada potensi pangkat jika ada "^", digit, atau tanda
    # minus (biasa/unicode) yang langsung menempel ke huruf (pangkat negatif).
    _has_potential_power = (
        '^' in full_text
        or re.search(r'\d', full_text)
        or re.search(r'[A-Za-zµΩ·⁻][\u2212\-]\d', full_text)
    )
    if not _has_potential_power:
        # Tidak ada potensi pangkat — tapi mungkin tetap ada perubahan
        # dari substitusi derajat di atas.
        original_text = ''.join(r.text for r in para.runs)
        if full_text != original_text and para.runs:
    ...
    # Kumpulkan rentang (start, end, exponent_text) yang harus jadi superscript,
    # serta teks dasar (sebelum diubah) yang harus ditulis ulang akibat
    # penghapusan simbol "^".
    spans = []  # list of (start, end, replacement_text_for_this_span, is_super)

    # Cek pola caret dulu (lebih spesifik), lalu unit power dan pangkat
    # negatif inline pada sisa teks yang belum kena match caret.
    consumed = [False] * len(full_text)

    for m in _RE_CARET_POWER.finditer(full_text):
        base_start, base_end = m.start(1), m.end(1)
        exp_start, exp_end = m.start(2), m.end(2)
        if any(consumed[base_start:exp_end]):
            continue
        for i in range(base_start, exp_end):
            consumed[i] = True
        spans.append((base_start, base_end, m.group(1), False))
        # Karakter '^' di antara base dan eksponen harus dihapus dari hasil
        # akhir — direpresentasikan sebagai span kosong (bukan dibiarkan
        # tertulis apa adanya).
        if base_end < exp_start:
            spans.append((base_end, exp_start, '', False))
        spans.append((exp_start, exp_end, m.group(2), True))

    for m in _RE_UNIT_POWER.finditer(full_text):
        unit_start, unit_end = m.start(1), m.end(1)
        pow_start, pow_end = m.start(2), m.end(2)
        if any(consumed[unit_start:pow_end]):
            continue
        for i in range(unit_start, pow_end):
            consumed[i] = True
        spans.append((unit_start, unit_end, m.group(1), False))
        spans.append((pow_start, pow_end, m.group(2), True))

    # (e) Pangkat negatif inline: mis. "kWm−2", "W·m−2", "s−1"
    # Tanda minus (biasa "-" atau unicode U+2212 "−") + digit langsung
    # setelah huruf/satuan — seluruh bagian "−N" dijadikan superscript.
    for m in _RE_INLINE_NEG_POWER.finditer(full_text):
        exp_start, exp_end = m.start(1), m.end(1)
        if any(consumed[exp_start:exp_end]):
            continue
        for i in range(exp_start, exp_end):
            consumed[i] = True
        spans.append((exp_start, exp_end, m.group(1), True))
    ...
    # Bangun ulang teks final + daftar segmen (text, is_super), menghilangkan
    # karakter '^' (yang tidak masuk span manapun di antara base & exponent).
    segments = []  # (text, is_super)
    cursor = 0
    for start, end, seg_text, is_super in spans:
        if start > cursor:
            segments.append((full_text[cursor:start], False))
        segments.append((seg_text, is_super))
        cursor = end
    if cursor < len(full_text):
        segments.append((full_text[cursor:], False))

    # Gabungkan segmen non-super yang bersebelahan supaya run tidak terlalu
    # terpecah-pecah secara berlebihan.
    merged = []
    for seg_text, is_super in segments:
        if not seg_text:
            continue
        if merged and merged[-1][1] == is_super:
            merged[-1] = (merged[-1][0] + seg_text, is_super)
        else:
            merged.append((seg_text, is_super))

    if len(merged) <= 1:
        return False

    # Ambil font referensi dari run pertama yang berisi teks.
    ...
    for seg_text, is_super in merged:
        nr = para.add_run(seg_text)
        if font_name: nr.font.name = font_name
        if font_size: nr.font.size = font_size
        if is_super:
            nr.font.superscript = True

    return True


def _fix_all_superscripts(doc: Document) -> None:
    for para in doc.paragraphs:
        _apply_superscript_to_para(para)
    for table in doc.tables:
        for row in table.rows:
            for cell in row.cells:
...
# ─────────────────────────────────────────────────────────────────────────────
# FITUR 5: PROTEKSI JUDUL STANDAR DI ACUAN NORMATIF & BIBLIOGRAFI
# ─────────────────────────────────────────────────────────────────────────────
# Pola: "ISO/IEC/SNI NNNNN[-N][, judul...]" atau "[N] ISO/IEC/SNI NNNNN..."
# Judul setelah nomor standar TIDAK boleh diterjemahkan — harus tetap dalam
# bahasa Inggris persis seperti aslinya. Fungsi ini mengembalikan teks dengan
# judul standar yang dilindungi sebagai placeholder, serta dict restorenya.
#
# Contoh:
#   "ISO 25711, Railway applications — Vocabulary for fire safety"
#   → "ISO 25711, §STD0§"  (placeholder)
#   restore: {"§STD0§": "Railway applications — Vocabulary for fire safety"}

_RE_STD_NUMBER = re.compile(
    r'(?:^\[\d+\]\s*)?'                        # opsional: "[N] " di awal
    r'((?:ISO|IEC|SNI|IEEE|ASTM|EN|BS|DIN|JIS|ANSI)'
    r'(?:/(?:IEC|ISO|SNI|TS|TR|PAS|GUIDE))*'   # mis. ISO/IEC, ISO/TR
    r'(?:\s+(?:GUIDE|TS|TR|PAS))?'             # mis. ISO GUIDE 73
    r'\s+\d+(?:[:\-]\d+)*(?:\s+\(\d{4}\))?)'  # nomor + bagian + tahun opsional
    r'(?:,\s*(.+))?$',                          # ", judul ..." opsional
    re.IGNORECASE
)

def _protect_std_titles_in_bib(text: str) -> tuple[str, dict]:
    """
    Jika baris teks adalah entri referensi standar (ISO/IEC/SNI/dll.),
    lindungi judul aslinya (teks setelah koma pertama) dari terjemahan.
    Kembalikan (teks_dengan_placeholder, restore_dict).
    Jika tidak cocok, kembalikan (teks_asli, {}).
    """
    t = text.strip()
    m = _RE_STD_NUMBER.match(t)
    if not m or not m.group(2):
        return text, {}
    std_title  = m.group(2)   # mis. "Railway applications — Vocabulary..."
    placeholder = '\xa7STD0\xa7'
    # Pertahankan pemisah ", " sebelum placeholder agar format tetap konsisten
    protected = t[:m.start(2)] + placeholder
    return protected, {placeholder: std_title}

def _restore_std_titles(text: str, restore_map: dict) -> str:
...
def _insert_original_before_bib(translated_doc: Document, orig_body_els: list, progress_callback=None) -> None:
    ...
    # Terapkan superscript (pangkat & pangkat negatif) pada konten asli yang
    # akan disisipkan — supaya "20 kWm−2" dll. juga tampil benar di bagian
    # standar asli berbahasa Inggris.
    _notify(progress_callback, 42, "Fix superscript konten asli...")
    for el in new_content_els:
        if el.tag != f'{_W}p':
            continue
        # Bungkus element sementara sebagai paragraf python-docx agar dapat
        # memanfaatkan _apply_superscript_to_para. Cara termudah: buat
        # Document dummy dan akses paragraf pertamanya.
        try:
            import tempfile, io as _io
            from docx import Document as _Doc
            _tmp = _Doc()
            _tmp.element.body.insert(0, copy.deepcopy(el))
            if _tmp.paragraphs:
                _apply_superscript_to_para(_tmp.paragraphs[0])
                # Salin kembali run yang sudah diperbaiki ke element asli
                fixed_el = _tmp.element.body[0]
                # Ganti konten run di el dengan yang sudah difix
                for old_r in el.findall(f'{_W}r'):
                    el.remove(old_r)
                for new_r in fixed_el.findall(f'{_W}r'):
                    el.append(copy.deepcopy(new_r))
        except Exception:
            pass  # Gagal superscript pada satu elemen tidak menghentikan proses
...
# ── API Keys Google Cloud Translate (isi minimal 1, sisanya bisa dikosongkan) ──
# Jika key utama (index 0) gagal/kuota habis, otomatis fallback ke key berikutnya.
GOOGLE_CLOUD_TRANSLATE_API_KEYS = [
    "AIzaSyA4oZctr_AkUAtwoyiDHM-Jp6OGBlI8TMc",  # Key 1  ← wajib diisi
    "",  # Key 2  (fallback)
    "",  # Key 3  (fallback)
    "",  # Key 4  (fallback)
    "",  # Key 5  (fallback)
...
class _Translator:
    def __init__(self, source: str = 'auto', target: str = 'id',
        ...
        try:
            ...
            raise ImportError("Jalankan: pip install requests")
        self._keys = [k for k in GOOGLE_CLOUD_TRANSLATE_API_KEYS if k and k.strip()]
        if not self._keys:
            raise ValueError("Tidak ada GOOGLE_CLOUD_TRANSLATE_API_KEYS yang terisi di engine9.py")
        self.source = source; self.target = target
        self.custom_dict = custom_dict; self.italic_dict = italic_dict
        self._api_url = "https://translation.googleapis.com/language/translate/v2"
        self._key_index = 0  # indeks key aktif saat ini
    ...
    def _call_api(self, t: str) -> str:
        """Coba satu request dengan key aktif; raise Exception jika gagal."""
        import requests
        params = self._build_params(t)
        resp = requests.post(self._api_url, params=params, timeout=10)
        resp.raise_for_status()
        return resp.json()["data"]["translations"][0]["translatedText"]

    def translate_one(self, text: str, italic_map: dict = None) -> tuple[str, list[str]]:
        """
        Terjemahkan satu teks via Google Translate API secara langsung.
        CATATAN: proteksi kamus (custom_dict/italic_dict) TIDAK dilakukan di
        sini — caller (_translate_para / _translate_hyperlinks_in_para)
        sudah menerapkan placeholder §N§ (italic) dan §TKn§ (custom
        dict) SEBELUM memanggil fungsi ini, dan akan me-restore-nya sendiri
        setelah menerima hasil. Melakukan tokenisasi lagi di sini hanya akan
        menggandakan proses tanpa manfaat.
        """
        t = text.strip()
...
def _translate_para(para, tr, past_bibliography: bool = False) -> list[str]:
    ...
    Alur tokenisasi yang BENAR:
    1. Italic dict  → ganti term dengan placeholder NUMERIK  §0§, §1§, …
       (bukan @@IT_...@@ karena Google Translate mengubah format itu)
    2. Custom dict  → ganti term dengan placeholder §TK0§, §TK1§, … lalu kirim ke API
    3. API Google Translate dipanggil dengan teks yang sudah di-proteksi
    4. Restore §TKn§ → terjemahan kamus
    5. Restore §N§ → teks asli (tidak diterjemahkan) → kumpulkan untuk italic
    ...
    # 2. Kumpulkan teks dari runs biasa (bukan hyperlink)
    ...
    # Ambil font dari run pertama yang ada teks
    font_name = 'Arial'; font_size = None
    for run in para.runs:
        if run.text.strip():
            if run.font.name: font_name = run.font.name
    ...
    # ── TAHAP A: Proteksi italic dict dengan placeholder §N§ ──────────────────
    # Placeholder §N§ lebih aman — Google Translate tidak mengubahnya.
    italic_slots = []   # [(index, original_term), ...]
    working = combined

    # Proteksi khusus acuan normatif/bibliografi:
    # Judul standar setelah nomor (mis. "ISO 25711, Railway applications…")
    # TIDAK boleh diterjemahkan — tetap dalam bahasa Inggris.
    _std_restore = {}
    working, _std_restore = _protect_std_titles_in_bib(working)
    ...
    if tr.italic_dict and len(tr.italic_dict) > 0:
        ...
        for term_lower, term_orig in sorted_italic:
            ...
            def _replacer(m, _orig=term_orig):
                idx = len(italic_slots)
                italic_slots.append(_orig)
                return f'\xa7{idx}\xa7'   # §N§  (\xa7 = §)
            working = pattern.sub(_replacer, working)

    # ── TAHAP B: Proteksi custom dict (kamus SNI) dengan placeholder §TKn§ ─────
    tk_map = {}
    if tr.custom_dict and len(tr.custom_dict) > 0:
        working, tk_map = tr.custom_dict._apply_pre(working)

    # ── TAHAP C: Panggil Google Translate ─────────────────────────────────────
    # Cek apakah masih ada teks nyata (di luar placeholder dan token)
    probe = re.sub(r'\xa7TK\d+\xa7', '', working)
    probe = re.sub(r'\xa7\d+\xa7', '', probe).strip()
    _ONLY_PUNCT = re.compile(
        r'^[\s\d\.\,\:\;\-\(\)\[\]\/\\\+\=\*\%\&\^\$\#\@\!\"\'`~<>{}|_©]*$'
    ...
    if probe and not _ONLY_PUNCT.fullmatch(probe):
        translated_raw = working
        for attempt in range(len(tr._keys)):
            try:
                import requests as _req
                params = {
                    "q": working,
                    "target": tr.target,
                    "key": tr._keys[tr._key_index],
                    "format": "text",
                }
                if tr.source and tr.source != 'auto':
                    params["source"] = tr.source
                resp = _req.post(tr._api_url, params=params, timeout=10)
                resp.raise_for_status()
                translated_raw = resp.json()["data"]["translations"][0]["translatedText"]
                if not translated_raw:
                    translated_raw = working
                break
            except Exception:
                time.sleep(0.5)
                tr._key_index = (tr._key_index + 1) % len(tr._keys)
        else:
            translated_raw = working
        time.sleep(_TRANSLATE_DELAY)
    else:
        # Seluruh teks ada di kamus / italic dict → tidak perlu panggil API
        translated_raw = working

    # ── TAHAP D: Restore token custom dict → terjemahan yang ditentukan ────────
    if tk_map:
        translated_raw = tr.custom_dict._apply_post(translated_raw, tk_map)

    # ── TAHAP D2: Restore judul standar yang dilindungi ───────────────────────
    if _std_restore:
        translated_raw = _restore_std_titles(translated_raw, _std_restore)

    # ── TAHAP E: Restore placeholder italic §N§ → teks asli ───────────────────
    italic_terms_found = []
    def _restore_italic(m):
        try:
            idx = int(m.group(1))
            orig = italic_slots[idx]
            if orig not in italic_terms_found:
                italic_terms_found.append(orig)
            return orig
        except (IndexError, ValueError):
            return m.group(0)

    translated = re.sub(r'\xa7(\d+)\xa7', _restore_italic, translated_raw)

    # Jika hasil sama dengan input asli dan tidak ada italic terms → tidak ada perubahan
    if not translated or (translated == combined and not italic_terms_found):
        if has_hl:
            _translate_hyperlinks_in_para(para, tr)
        return []
    ...
    # ── TAHAP F: Terapkan teks + format italic ke paragraf ────────────────────
    ...
    # ── TAHAP G: Terjemahkan hyperlink SECARA TERPISAH ────────────────────────
    if has_hl:
        _translate_hyperlinks_in_para(para, tr)
...
# Pola "Bagian 1:" / "Part 1:" — dipakai untuk menormalkan spasi setelah
# titik dua jadi tepat satu spasi, SEKALIGUS mengkapitalisasi huruf pertama
# kata setelahnya, KHUSUS pada pola ini saja (bukan setiap titik dua). Grup 1
# menangkap "Bagian N"/"Part N" TANPA titik dua, supaya spasi sebelum ":"
# ikut dirapikan.
_RE_BAGIAN_PART = re.compile(r'((?:Bagian|Part)\s+\d+)\s*:\s*([a-zA-Z])')

# Strip PANJANG (en dash "–" atau em dash "—") sebagai pemisah antar-klausa
# pada judul. Hyphen pendek "-" SENGAJA tidak disertakan karena umumnya
# bagian dari kata majemuk (mis. "non-destructive") atau nomor standar
# (mis. "14001-1") yang tidak boleh diutak-atik spasinya.
...
def _normalize_title_line(text: str) -> str:
    """
    Judul pada cover/dokumen ISO sumber sering memuat:
    1. Line-break manual (<w:br/>) di tengah judul panjang, mis. "...— Bagian
       ...
       tertulis ulang sebagai <w:br/> lagi saat judul disalin ke body
       title/prakata (lewat para.add_run di _replace_para_text), membuat
       judul terlihat terpecah jadi beberapa baris padahal seharusnya satu
       baris penuh.
    2. Strip pemisah klausa yang tidak konsisten (en dash/em dash dengan
       spasi yang juga tidak konsisten).
    ...
    2. Merapikan strip panjang (en dash/em dash) jadi " — ", lalu
       mengkapitalisasi huruf pertama kata sesudahnya. Hyphen pendek "-"
       TIDAK disentuh sama sekali.
    3. Merapikan spasi di sekitar "Bagian N:"/"Part N:" jadi tepat satu
       spasi setelah titik dua, sekaligus mengkapitalisasi huruf pertama
       kata setelahnya.
    """
    if not text:
        return text
...
def _extract_cover_titles(doc: Document) -> tuple[str, str]:
    ...
    return _normalize_title_line(id_title), _normalize_title_line(en_title)
...
class DocxFinalTranslatorEngine:
    ...
    def translate(self, input_docx: str, output_docx: str, progress_callback=None, 
        ...
        try:
            ...
            _fix_all_notes(doc)
            _notify(progress_callback, 68, "Fix superscript pangkat...")
            _fix_all_superscripts(doc)
