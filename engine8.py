...
class _Translator:
    def __init__(self, source: str = 'auto', target: str = 'id'):
        ...
        if not self._keys:
            raise ValueError("Tidak ada GOOGLE_CLOUD_TRANSLATE_API_KEYS yang terisi di engine8.py")
        self.source = source
        self.target = target
    ...
    def _try_request(self, t: str) -> str:
    ...
    def translate_one(self, text: str) -> str:
        ...
        last_exc = None
        # Iterasi semua key yang tersedia sebagai fallback
        for attempt in range(len(self._keys)):
            try:
                result = self._try_request(t)
                return result if result else text
            except Exception as e:
                last_exc = e
                time.sleep(0.5)
                # Pindah ke key berikutnya (melingkar)
                self._key_index = (self._key_index + 1) % len(self._keys)

        # Semua key gagal → kembalikan teks asli
        return text
